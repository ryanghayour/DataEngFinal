#!/bin/bash
# sweep_experiments_reuters_filterlabel.sh
# Runs all 18 LTS experiments on the Reuters-21578 crude oil dataset.
...
RESULTS_DIR="$PROJECT_DIR/sweep_results_reuters_filterlabel_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_DIR"
...
run_experiment() {
    ...
    # All three clustering methods are stochastic -- delete every cache
    # so each run gets fresh clusters (apples-to-apples variance comparison).
    rm -f data_use_cases/data_reuters_crude_lda.csv
    rm -f data_use_cases/data_reuters_crude_bertopic.csv
    rm -f data_use_cases/data_reuters_crude_top2vec.csv
    ...
    if [ -f "data_use_cases/data_reuters_crude_model_results.json" ]; then
        cp data_use_cases/data_reuters_crude_model_results.json "$results_file"
    fi
